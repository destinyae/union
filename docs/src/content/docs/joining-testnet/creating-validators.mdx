...
As we get closer to a mainnet release, we will update staking and slashing parameters and ask for more realistic self delegations. In the meantime, we will ensure that testnet is configured to change quickly with minimum friction.
...
If your node isn't set up to accept RPC requests, you can send them to another node via the `--node` option.
